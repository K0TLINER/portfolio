    ...
    <>
      <Card>
        ...
        <CardContent>
          ...
          <Typography variant="body2" color="text.secondary">
            <Link
              href="https://www.youtube.com/watch?v=ySCuN_o_ssg&t=1260s"
              target="_blank"
            >
          ...
          <Typography variant="body2" color="text.secondary">
            <Link
              href="https://youtu.be/QbFXbEj1Bzg?si=ntEjViXUttWVique"
              target="_blank"
            >
        ...
        <Collapse in={expanded} timeout="auto" unmountOnExit>
          <CardContent>
            ...
            <Typography paragraph>
              ...
              나의 경험에 도움이 될 만한 것이 무엇이 있을까 찾는 중에 Naver
              Cloud에서 지원하는 과정인 DevOps 2기를 지원했습니다. 비전공자도
              많이 하는 과정이라고 들었지만, 저한테 가장 메리트가 있다고 생각이
              든 점은 바로 Naver Cloud 크레딧 300만원 지원이었습니다.
              <br />
              AWS ec2 free 플랜으로 사용해 봤던 저는 학생으로서 돈이 없어서 유료
            ...
            <Typography paragraph>
              ...
              <br />
              크게 느낀 부분은 Spring 파트였습니다. 저는 Spring boot를 사용해
              왔었는데 JAVA의 Servlet파트와 Spring을 배우고 나니 Spring boot가
              좀 더 친근하게 다가오게 되었습니다. Servlet과 WAS로 웹 환경이
              구성되었을 때와 비교하여, Spring 이라는 Framework를 사용한다면
              코드에 대부분을 메타데이터인 xml로 처리하는 것과 IOC에 필요성에
              대해 경험하였고 xml도 많아지므로 Spring boot 또한 더 메타데이터를
              줄이는 이 흐름 자체가 어느 곳에서도 경험해 볼 수 없었지만 이
              과정에서 경험을 할 수 있어서 저는 너무 좋았습니다.
              <br />
              저는 흐름이 있어야지 이해하기도 편하고 기억도 잘하는 편이라서 이
              경험으로 인해 문제해결 능력이 많이 향상된 것 같습니다.
            </Typography>
            <Typography paragraph>
              이 과정에서 개인 프로젝트를 진행했는데 그때 12번의 리펙터링을
              진행했습니다.
            </Typography>
            <CardMedia
              component="img"
            ...
            <Typography paragraph>
              이에 따라 자바 기술의 초창기 웹으로 시작해서 현재 많이 사용되는
              스프링 부트까지 어떤 점이 보완되었으며 어떤 이유로 인해 새로운
              기술이 탄생하는지 피부로 직접 느낄 수 있었습니다.
              <br />
              대학 과정에서는 수강 신청을 통해 과목을 배우기 때문에 필요에 의해
              또는 어떤 이유로 이 과목을 수강해야 하는지 배울 수 없었는데 배울
              수 있어서 좋았습니다.
            </Typography>
            <Typography paragraph>
              그 이후 6주 정도 Linux와 nCloud 제품들을 배웠습니다. 또 Docker,
              Jenkins, React를 배웠는데 이 기술 같은 경우는 깊게 하지는 않았고
            ...
            <Typography paragraph>
              ...
              분석설계단계에서는 팀에서 이전부터 생각해 놓고 있던 구현 주제를
              명확히 합니다. 그 이후 현업요구사항 정의서, Usecase 정의서,
              Usecase Diagram, Usecase 추적표, Class Diagram, ERD Diagram 등을
              작성하는 단계입니다. 구현에만 신경을 썼던 학생 시절에는 이 단계를
              오랫동안 생각하지 않았습니다. 그러다 보니 서로 말이 안 맞아서
              시간이 지체되는 경우가 있었는데 이 과정에서 많은 커뮤니케이션을
              한다면 그런 상황은 적어질 거라고 느꼈습니다.
            ...
            <Typography paragraph>
              ...
              <br />
              상대방에 읽음 여부, 채팅을 치고 있다면 채팅을 치고 있다고 표시
              하고, 실시간 알림, 채팅방 이미지 업로드, 약속 잡기, 위치 공유,
              송금 및 송금 요청하기 기능을 개발했습니다. 모바일 웹에서 현재
              위치를 얻어야 하므로 https환경으로 배포 환경을 구축했습니다.
              그리고 실시간 영상 통화 기능, 채팅방 나가기 및 차단 기능으로
              채팅방을 관리했습니다. 채팅방은 MySQL로 관리하기 때문에 다른
              서비스 간 REST API요청을 통해서 데이터를 주고받았습니다. 구현한
              내용은 Youtube에 업로드했습니다. 발표 자료였기 때문에 모든 내용이
              들어가지는 못했지만, 핵심 기능은 어느 정도 들어갔습니다. 하지만
              아쉬운 점은 채팅방과 메시지가 다른 서비스의 DB로 구성되어 있기
              때문에 트랜잭션에 관한 문제는 해결하지 못했습니다. MSA를
              공부하면서 해결해야 할 과제 중 하나입니다.
            </Typography>
            <Typography paragraph>
            ...
            <Typography paragraph>
              여기서 사용된 도커 이미지 및 코드는 Docker Hub와 Github에
              업로드해서 보관했습니다.
            </Typography>
            <Typography paragraph>
              이 과정을 통해서 Docker와 Jenkins를 사용하는 것에 많이
              익숙해졌지만, 아직 많이 부족합니다. MSA에 대해서 확실하게 가닥을
              잡으려고 노력하고 K8S도 필요성을 느끼고 깊게 공부해 보고 싶습니다.
              kafka도 마찬가지로 필요성을 느낄 수 있도록 많은 경험을 쌓도록
              노력하겠다고 생각했습니다.
            </Typography>
          </CardContent>
        </Collapse>
